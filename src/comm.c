...
/* RT code for breaking link */
	...
	write_to_buffer( d, "\n\r", 2 );
		...
		write_to_buffer(d, "Icey Depths is home for the following races:\n\r", 0);
	...
	case CON_GET_NEW_RACE:
		...
		if (!str_cmp(arg, "help")) {
			...
			if (argument[0] == '\0') {
				write_to_buffer(d, "Icey Depths is the home for the following races:\n\r", 0);
	  			do_help(ch,"RACETABLE");
			}
			else {
				do_help(ch, argument);
				write_to_buffer(d, "What is your race? ('help <race>' or help table for more information) ",0);
			}	
			break;
		}
		...
		if (race == 0 || !r->pcdata) {
			...
			write_to_buffer(d, "\n\r", 0);
			write_to_buffer(d, "What is your race? ('help <race>' for more information or help table for more information) ", 0);
			break;
		}
	...
	case CON_CREATE_DONE:
		...
		info (NULL,0,"{B[{WINFO{B]{x {R%s{x is a new player!\r\n",ch->name);
		do_help(ch, "1.newbie");
		break;
...
if (!IS_IMMORTAL(ch))
info( NULL,0,"{B[{WInfo{B]{x %s has entered the game.\r\n", ch->name);
...
#define NEWBIE_TOKEN 3741
			obj_to_char(create_obj(get_obj_index(NEWBIE_TOKEN), 0), ch);
