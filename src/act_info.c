...
void do_who_raw(CHAR_DATA* ch, CHAR_DATA *wch, BUFFER* output)
{
	...
	&&  (!IS_SET(clan->flags, CLAN_HIDDEN) ||
		...
		buf_printf(output, "[{M%s{x] ", clan->name);
...
DO_FUN(do_who)
{
	 ...
	 */
	output = buf_new(ch->lang);
buf_add(output,"{CL{cevel  {RR{race  {CCl{Dass{x\r\n");
buf_add(output,"{D------------------{x\r\n");
	...
	buf_printf(output, "{x\nPlayers found: %d. Most since last reboot: %d.\n",
		   nMatch, max_on);
	page_to_char(buf_string(output), ch);
	buf_free(output);
...
void do_practice(CHAR_DATA *ch, const char *argument)
{
	...
	if (argument[0] == '\0') {
		...
		for (i = 0; i < ch->pcdata->learned.nused; i++) {
			...
			buf_printf(output, "{G%-18s {W%3d%% {x ",
				   sk->name, ps->percent);
			if (++col % 3 == 0)
				buf_add(output, "\n");
		}
...
for (cmd = 0; social_table[cmd].name != NULL; cmd++) {
    ...
    if (!found){
	found = FALSE;
	send_to_char("sorry thats not a gocial\r\n",ch);
	return;
	}
    ...
    if (arg[0] == '\0')
    {
	sprintf(buf, "{M[{WGocial{M]{x: %s",
social_table[cmd].val[SOC_CHAR_NO_ARG]
);
	act_puts(buf,ch,NULL,NULL,TO_CHAR,POS_DEAD);
	sprintf(buf, "{M[{WGocial{M]{x: %s",social_table[cmd].val[SOC_OTHERS_NO_ARG] );
	for (d = descriptor_list; d != NULL; d = d->next)
	{
	    CHAR_DATA *vch;
	    ...
	    {
		act_puts(buf,ch,NULL,vch,TO_VICT,POS_DEAD);
	    }
	}
    }
    ...
    {
	sprintf(buf,"{M[{WGocial{M]{x: %s",
social_table[cmd].val[SOC_CHAR_AUTO]);
	act_puts(buf,ch,NULL,NULL,TO_CHAR,POS_DEAD);
	sprintf(buf,"{M[{WGocial{M]{x: %s",
social_table[cmd].val[SOC_OTHERS_AUTO]);
	for (d = descriptor_list; d != NULL; d = d->next)
	{
	    CHAR_DATA *vch;
	    vch = d->original ? d->original : d->character;
    ...
    {
	sprintf(buf,"{M[{WGocial{M]{x: %s",
social_table[cmd].val[SOC_CHAR_FOUND]);
	act_puts(buf,ch,NULL,victim,TO_CHAR,POS_DEAD);
	sprintf(buf,"{M[{WGocial{M]{x: %s",
social_table[cmd].val[SOC_VICT_FOUND]);
	act_puts(buf,ch,NULL,victim,TO_VICT,POS_DEAD);
	
	sprintf(buf,"{M[{WGocial{M]{x: %s",social_table[cmd].val[SOC_OTHERS_FOUND]);
