...
void do_immtalk(CHAR_DATA *ch, const char *argument)
{
	...
	act_puts("{R[{WIMM{R]{x $n: {R$t{x", ch, argument, NULL, TO_CHAR,
POS_DEAD);
	for (d = descriptor_list; d; d = d->next) {
		...
		&&  !IS_SET(victim->comm, COMM_NOWIZ))
			act_puts("{R[{WIMM{R]{x $n: {R$t{x", ch, argument,
d->character,
...
void do_gossip(CHAR_DATA *ch, const char *argument)
{
	...
	if (ch->level < 5) {
		char_puts("Sorry you must be level {R5{x to gossip. Try 'Newbie' instead.\n",ch);
		return;
	}
	if (IS_SET(ch->comm, COMM_NOCHANNELS)) {
		 char_puts("The gods have revoked your channel privileges.\n", ch);
...
void do_quit_org(CHAR_DATA *ch, const char *argument, bool Count)
{
	...
	if (!IS_IMMORTAL(ch))
	info (NULL,0,"{B[{WINFO{B]{R %s{x has left Icey Depths.\r\n",ch->name);
	/*
...
void do_group(CHAR_DATA *ch, const char *argument)
{
	...
	if (ch->level - victim->level < -8 || ch->level - victim->level
> 8) {
...
void info( CHAR_DATA * ch, int level, char * message, ...){
    char buf[MAX_STRING_LENGTH];    DESCRIPTOR_DATA * d;    va_list args;
    va_start(args,message);    vsnprintf(buf, MAX_STRING_LENGTH, message,args);
    va_end(args);    for ( d = descriptor_list; d; d = d->next )    {
        if ( d->connected == CON_PLAYING &&              d->character !=ch )
           send_to_char(buf, d->character);    }}
void do_info(CHAR_DATA * ch, char * argument){
    if (IS_SET(ch->comm,COMM_NOINFO))    {
        send_to_char("Info channel is now ON.\n\r",ch);
        REMOVE_BIT(ch->comm,COMM_NOINFO);    }    else    {
...
void do_newbie(CHAR_DATA *ch, const char *argument)
{
	...
	argument = garble(ch, argument);
	act_puts("{g[{WN{Ge{Ww{Gb{Wi{Ge{g]{x $n: '$t'",
		 ch, argument, NULL,
		 TO_CHAR | ACT_NODEAF, POS_DEAD);
