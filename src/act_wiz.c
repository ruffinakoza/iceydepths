...
void do_transfer(CHAR_DATA *ch, const char *argument)
{
	...
	if (victim->level > ch->level) {
		char_puts("Sorry you cant do that to a higher level Imm.\n", ch);
		return;
	}
...
"{C:{B==========================================================================={C:{x\n\r");    for ( d = descriptor_list; d; d = d->next )    {
        if ( d->character && can_see( ch, d->character ) )        {
           ...
           switch( d->connected )           {
              ...
              case CON_GET_ALIGNMENT:   	 st = "New Aln ";
break;
              case CON_DEFAULT_CHOICE:		 st = "Default ";
break;
              case CON_GEN_GROUPS:			 st = " Custom ";
break;
...
break;
           ...
           sprintf( buf, "{B|  [{C%-3d %-8s %7s  %2s{B]  {C%-12s  %-30s {B|{x\n\r",              d->descriptor,              st,              s,
              idle,              ( d->original ) ? d->original->name
                              : ( d->character )  ? d->character->name
                                                  : "(None!)",
              d->host );                         strcat( buf2, buf );
}
    }    strcat( buf2, 
"{B|                                                                           		|{x");
sprintf( buf, "\n\r{B|  Users:{C%-2d {B							|{x\n\r", count );    
strcat( buf2, buf );    strcat( buf2,
"{C:{B==========================================================================={C:{x");    send_to_char( buf2, ch );    return;}
/*
...
void do_reboot(CHAR_DATA *ch, const char *argument)
{
	...
	if (is_number(arg)) {
		...
		rebooter = 1;
		char_printf(ch, "Icey Depths will reboot in %i ticks.\n",
			    reboot_counter);
		return;
	}
...
void reboot_muddy(void)
{
	...
	for (d = descriptor_list; d != NULL; d = d_next) {
		d_next = d->next;
		write_to_buffer(d,"Icey Depths is going down for rebooting NOW!",0);
		if (d->character)
